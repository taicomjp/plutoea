#!/usr/bin/perl
use encoding 'utf8', STDIN => 'cp950', STDOUT => 'utf8';
$db = "item_db";
$nb_columns = 22;
...
sub printField {
	...
	if ($str =~ /{.*}/)
	{
		...
		# 移除前後空白
		my $strspace = " ";
		my $strspacefirst = index($str, $strspace);
		if ($strspacefirst == 0){$str = substr($str,$strspacefirst+1);}
		my $strspacelast = rindex($str, $strspace);
		if ($strspacelast == length($str)-1){$str = substr($str,0,$strspacelast);}
	}
	# If nothing, put NULL
	if ($str eq "") {
